...
def analyze_article_no_claim(title: str, content: str) -> Dict[str, Any]:
    ...
    ────────────────────────
    2. JAPANESE NEWS ARTICLE GENERATION
    ────────────────────────
...
def analyze_article1(title: str, content: str) -> Dict[str, Any]:

    system_prompt = """
    TASK:
    You are a professional newspaper reporter and editor writing for the Japanese edition of a global news media outlet.

    Analyze the given news title and content and generate new title and content. Must return structured data in STRICT JSON format only.
    Do not include explanations, markdown, or extra text.


    1. JAPANESE NEWS ARTICLE GENERATION
    ────────────────────────
    ...
    Language and style rules:
    - rewrite in proper newspaper prose
    - Keep each sentence within 90 characters whenever possible
    - Do NOT use the kanji 「述」 in any form
    - Write のべる / のべた / のべている in hiragana only
    - After using のべた once, vary expressions (とした, と語った, だとした)
...
def extract_info(title: str, content: str) -> Dict[str, Any]:
  system_prompt = """
    TASK:
    You are a professional newspaper reporter and editor writing for the Japanese edition of a global news media outlet.
...
def generate_uhalisi_posts(title: str, content: str) -> Dict[str, Any]:
  system_prompt = """
    TASK:
    You are a professional newspaper reporter and editor writing for the Japanese edition of a global news media outlet.
